...
export default function PreviousClaimsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    setLoading(true);
    fetch("/api/claims/previous")
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data)) setClaims(data);
        else toast.error(data?.error ?? "Failed to load previous claims.");
      })
      .catch(() => toast.error("Failed to load previous claims."))
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <h1 className="text-3xl font-bold mb-1">Previous Claims</h1>
      <p className="text-muted-foreground mb-8">
        Items you have successfully claimed and received.
      </p>
      ...
      {!loading && claims.length === 0 && (
        <div className="text-center py-16 text-muted-foreground">
          <p className="text-lg font-medium">No previous claims yet.</p>
          ...
          {claims.map((claim) => (
            <Card key={claim.id}>
              ...
              <CardContent className="space-y-1 text-sm text-muted-foreground">
                ...
                {claim.claimed_item?.date_returned && (
                  <p>
                    <span className="font-medium text-foreground">
                      Date returned:
                    </span>{" "}
